  ...
  helper :authorize_net
	...
	def create
		...
		params['amount'].to_i.times do
			...
			if @cart.save
				...
				Room.where(hotel_id: params['hotel_id'], room_type: params['room_type']).first.destroy
			else
	...
	def checkout
	  ...
	  if @books.length > 0
		...
		UserMailer.admin_payment_pending(@user, @admin5).deliver_now
		...
		if @team.exempt
			if @team.books.length < 5
				...
				@team.users.each do |val|
					...
					UserMailer.tournament_confirmation(val, @admin5, @transaction).deliver_now
				end
